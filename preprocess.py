...
def render():
    ...
    with st.container(border=True):
        ...
        with col_reup:
            if st.button("🔄 Upload New File", type="secondary", use_container_width=True, key="reupload_btn"):
    ...
    with f2:
        flt_grp   = st.selectbox("Group", ["All"] + sorted(disp_stats["Group"].dropna().unique().tolist()), key="clf_flt_grp", label_visibility="collapsed")
    with f3:
        flt_dtype = st.selectbox("Demand Type", ["All", "smooth", "erratic", "intermittent", "insufficient"], key="clf_flt_dtype", label_visibility="collapsed")
    with f4:
        flt_stat  = st.selectbox("Status", ["All", "Passing Only", "Excluded Only"], key="clf_flt_stat", label_visibility="collapsed")
    with f5:
        flt_reset = st.button("🔄 Reset", key="clf_flt_reset", use_container_width=True)
...
def _render_upload_section():
    ...
    # (typing in search, clicking Reset, etc.), so we guard with the filename.
    # If _staged_df already exists for this filename, reuse it immediately.
    already_staged = (
    ...
    if not already_staged:
        ...
        # Commit to session state only on first read
        s._staged_df       = df
        s._staged_filename = uploaded.name
    else:
        # File already read — reuse cached dataframe, no disk I/O
        df = s._staged_df
    ...
    with col_btn1:
        if st.button("✅ Confirm & Run Imputation", type="primary", use_container_width=True, key="confirm_btn"):
...
def _render_sku_selection_inline(df, current_skus, key_suffix="inline"):
    ...
    if st.button("✅ Apply SKU Selection", type="primary", use_container_width=False, key=f"apply_sku_{key_suffix}"):
        s.selected_skus        = selected_skus
        s.n_skus               = len(selected_skus)
...
def _render_sku_selector(df, skus, n_skus, key_suffix=""):
    ...
    with col_b:
        if selection_method == "Manual selection":
            ...
            # ── SKU preview table — styled to match SKU Classification Table ──
            # Use a callback for Reset so it ONLY clears the search key —
            # no st.rerun() means the file is never re-read on reset or typing.
            ...
            display_skus = [s for s in top_skus if search_val.lower() in s.lower()] if search_val else top_skus
