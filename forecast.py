...
def _render_results(s, sku_sel=None):
    ...
    if not summary_df.empty:
        ...
        dtype_opts_ms = ["All Demand Types", "smooth", "erratic", "intermittent", "insufficient"]

        # Reset: set defaults BEFORE widgets are instantiated
        if st.session_state.pop("_ms_reset", False):
            st.session_state["ms_flt_sku"]   = ""
            st.session_state["ms_flt_model"] = model_opts[0]
            st.session_state["ms_flt_dtype"] = dtype_opts_ms[0]
        ...
        with ms_f3:
            ms_flt_dtype = st.selectbox("Demand Type", dtype_opts_ms, key="ms_flt_dtype", label_visibility="collapsed")
        with ms_f4:
            if st.button("🔄 Reset", key="ms_flt_reset", width="stretch"):
                st.session_state["_ms_reset"] = True
                st.rerun()
        ...
        if ms_flt_dtype != dtype_opts_ms[0] and "Demand Type" in tbl.columns:
